# Forest Plot of OpenPBTA miRNA cluster survival analysis results with short histologies as predictors
# Author: Shehbeel Arif
# Children's Hospital of Philadelphia
# Script adapted from S. Spielman for ALSF CCDL & Jo Lynne Rokita for D3b, 2022
...
output_dir <- file.path(root_dir, "analyses", "03-survival-analysis", "plots")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Input directory
input_dir <- file.path(root_dir, "analyses", "03-survival-analysis", "output")
...
forest_pdf <- file.path(output_dir, "forest_pbta_mirna_cluster.pdf")

coxfile <- readRDS("/Users/shehbeel/Documents/PBTA-miRNA-Analysis/analyses/03-survival-analysis/output/cox_cluster.RDS")

## Make plot --------------------------------------------------
ref_term <- "Cluster1"

# Set up ordering and labels for y-axis
term_order <- rev(c(ref_term,
                    "Cluster2",
                    "Cluster3",
                    "Cluster4"))

term_labels <- rev(c("Cluster1 (ref)",
                     "Cluster2",
                     "Cluster3",
                     "Cluster4"))
...
survival_df_spread <- survival_df %>%
  ...
  mutate(value = ifelse(term == "Cluster1 (ref)", NA, value))
